...
uint8_t returnStatus,sdcardType;
static volatile uint8_t pending=0;//volatile tells the compiler not to change its value as the value of this variable keeps on changing
char option[6],read_filename[12],write_filename[12];
char write_file_buf[64],read_file_buf[64];
static volatile uint8_t write_file_open=0;//volatile tells the compiler not to optimise the value of the variable
...
static void BulkOut(unsigned char bEP, unsigned char bEPStatus)
{
	...
	{
		iLen = USBHwEPRead(bEP,write_buffer, sizeof(abBulkBuf));//reads data from endpoint buffer
	}
	else
	{
		iLen = USBHwEPRead(bEP, abBulkBuf, sizeof(abBulkBuf));//reads data from endpoint buffer
	}
	lcd_clear();
	...
	else if(pending == 1)
	{
		...
		{
			strcpy(write_filename,abBulkBuf);//function copies the data of abBulkBuf to write_filename
			i=0;
			lcd_clear();
	...
	{		
		...
		{
			srcFilePtr = FILE_Open(write_filename,WRITE,&returnStatus);//function opens the file in write mode
			if(srcFilePtr == 0)
			{
				lcd_clear();
		...
		if(strcmp(write_buffer,"__END__") == 0)//check whether user has passed EOF
		{
			FILE_PutCh(srcFilePtr,EOF_FAT);//writes EOF to srcFilePtr
			FILE_Close(srcFilePtr);//function closes the file and release the memory used by the file
			write_file_open=0;
			pending=0;
			return;
		}
		...
		{
			FILE_PutCh(srcFilePtr,write_file_buf[i++]);//writing byte of data from write_file_buf to file
		}
	}
	...
	switch(pending)
	{
		case 2 ://READ FILE Operation pending
			if(FILE_READ)//read file name storing
			{
				...
				lcd_puts(LCD_LINE2,read_filename);
				srcFilePtr = FILE_Open(read_filename,READ,&returnStatus);//open the file for read operation
				if(srcFilePtr == 0)
				{
					lcd_clear();
				...
				else
				{
					...
					while(1)
					{
						...
						ch = FILE_GetCh(srcFilePtr);//reads a byte of data from file
						if(ch == EOF_FAT)
						{
							abBulkBuf[i]='\0';
						...
						if(i == 63)
						{
							USBHwEPWrite(bEP, abBulkBuf, 64);//writes data to abBulkBuf
							i=0;
						}

						abBulkBuf[i++]=ch;//read charater by character
						++bytes_read;
		...
		case 4 ://LIST Files operation Pending
			...
			while(1)
			{
				returnStatus = FILE_GetList(&fileList);//the function traverses through the sd card and list all the files present on it
...
int main(void)
{
	...
	returnStatus = SD_Init(&sdcardType);//Initialisation of sd card 
	...
	// initialise stack
	USBInit();//goto usb hardware
	/*first clock will be set to match system frequency with device frequency
	 * sytem clock will be matched with either device bus or AHB bus and 
	 * accordingly frequency will be set to system frequency and this is done
	 * with the help of pll i.e phase locked loop circuit 
	 * Initialise USB hardware*/


	// register descriptors
	USBRegisterDescriptors(abDescriptors);
	/* Passing the descriptors of the usb */


	// register endpoint handlers
	//	USBHwRegisterEPIntHandler(INT_IN_EP, NULL);
	USBHwRegisterEPIntHandler(BULK_IN_1_EP, BulkIn);
	/* when host will receive "IN" request the interrupt will be generated after
	 * which its entry will be done in vector interrupt table and after that
	 * its isr will be executed */

	USBHwRegisterEPIntHandler(BULK_OUT_1_EP, BulkOut);	
	/* when host will receive "OUT" request the interrupt will be generated after
	 * which its entry will be done in vector interrupt table and after that
	 * its isr will be executed 
	 * BulkOut function will be called*/

	// enable bulk-in interrupts on NAKs
	USBHwNakIntEnable(INACK_BI);
	/*this function is used to generate interrupt when NAK signal is received by the host */
	...
	USBHwConnect(TRUE);//connects from the usb bus
	lcd_clear();
